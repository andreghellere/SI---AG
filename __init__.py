...
class Individuals():
    ...
    def mutate(self, mutationRate):
        ...
        # sorteando um intervalo de 1% a 100%
...
class GeneticAlgorithm():
    ...
    def view_generation(self):
        ...
        print("Geração: %s -> Valor Fitness: %s Cromossomo: %s" % (
            best.generation,
    ...
    def resolve(self, mutationRate, generations, time_distances, cities):
        ...
        for generation in range(generations):
            ...
            for individual in self.population:
                individual.fitness()
                # Uncomment do debug
                print("Nova População %s - Fitness %s" %
...
def handler(event, context):
    ...
    try:
        ...
        print({
            'Geração': result[0],
            'Distância Percorrida': result[1],
            'Cromossomo': result[2],
            'Postos': c.chromose_to_cities(result[2])
        })
